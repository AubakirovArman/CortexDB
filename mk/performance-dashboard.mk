...
.PHONY: verify-performance-check numeric-verify-index-check temporal-validity-index-check graph-index-performance-check concurrent-read-benchmark-check ingestion-throughput-check ingestion-throughput-100k-check
...
ingestion-throughput-check:
	cargo run --release -p cortex-engine --bin ingestion_throughput_check -- --root "$(INGESTION_THROUGHPUT_ROOT)" --report "$(INGESTION_THROUGHPUT_REPORT)" --markdown "$(INGESTION_THROUGHPUT_MARKDOWN)" --docs "$(INGESTION_THROUGHPUT_DOCS)" --ingestion-batch-size "$(INGESTION_THROUGHPUT_BATCH_SIZE)" --embedding-batch-size "$(INGESTION_THROUGHPUT_EMBEDDING_BATCH_SIZE)" --payload-bytes "$(INGESTION_THROUGHPUT_PAYLOAD_BYTES)" --min-docs-per-sec "$(INGESTION_THROUGHPUT_MIN_DOCS_PER_SEC)"
ingestion-throughput-100k-check:
	cargo run --release -p cortex-engine --bin ingestion_throughput_check -- --root "$(INGESTION_THROUGHPUT_100K_ROOT)" --report "$(INGESTION_THROUGHPUT_100K_REPORT)" --markdown "$(INGESTION_THROUGHPUT_100K_MARKDOWN)" --docs "$(INGESTION_THROUGHPUT_100K_DOCS)" --ingestion-batch-size "$(INGESTION_THROUGHPUT_BATCH_SIZE)" --embedding-batch-size "$(INGESTION_THROUGHPUT_EMBEDDING_BATCH_SIZE)" --payload-bytes "$(INGESTION_THROUGHPUT_PAYLOAD_BYTES)" --min-docs-per-sec "$(INGESTION_THROUGHPUT_MIN_DOCS_PER_SEC)"
...
performance-trend-check:
	python3 scripts/performance_trend_check.py --load-report "$(LOAD_SMOKE_REPORT)" --single-node-report "$(SINGLE_NODE_PERF_REPORT)" --concurrent-read-report "$(CONCURRENT_READ_BENCH_REPORT)" --ingestion-throughput-report "$(INGESTION_THROUGHPUT_REPORT)" --history-root "$(PERFORMANCE_HISTORY_ROOT)" --report "$(PERFORMANCE_TREND_REPORT)"
continuous-benchmark-gate: ; $(MAKE) concurrent-read-benchmark-check && $(MAKE) ingestion-throughput-check && $(MAKE) performance-trend-check && $(MAKE) scale-bench-trends && $(MAKE) memory-estimate-audit && python3 scripts/continuous_benchmark_gate.py --min-regression-delta-ms "$(CONTINUOUS_BENCHMARK_MIN_REGRESSION_DELTA_MS)"
continuous-benchmark-hosted-gate: ; $(MAKE) load-smoke-check && $(MAKE) single-node-performance-check && $(MAKE) concurrent-read-benchmark-check && $(MAKE) ingestion-throughput-check && $(MAKE) scale-bench-ci && $(MAKE) performance-trend-check PERFORMANCE_HISTORY_ROOT="$(HOSTED_PERFORMANCE_HISTORY_ROOT)" && $(MAKE) scale-bench-trends && $(MAKE) memory-estimate-audit && python3 scripts/continuous_benchmark_gate.py --min-regression-delta-ms "$(HOSTED_BENCHMARK_MIN_REGRESSION_DELTA_MS)" && python3 scripts/continuous_benchmark_gate.py --self-test
