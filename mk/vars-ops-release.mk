...
NUMERIC_VERIFY_INDEX_ROOT ?= target/numeric-verify-index
NUMERIC_VERIFY_INDEX_REPORT ?= $(NUMERIC_VERIFY_INDEX_ROOT)/report.json
NUMERIC_VERIFY_INDEX_CELLS ?= 1000000
NUMERIC_VERIFY_INDEX_SAMPLES ?= 25
NUMERIC_VERIFY_INDEX_BATCH_SIZE ?= 50000
NUMERIC_VERIFY_INDEX_MAX_P95_MS ?= 250
NUMERIC_VERIFY_INDEX_SKIP_VALIDATION ?= --skip-validation
TEMPORAL_VALIDITY_INDEX_ROOT ?= target/temporal-validity-index/db
TEMPORAL_VALIDITY_INDEX_REPORT ?= target/temporal-validity-index/report.json
TEMPORAL_VALIDITY_INDEX_CELLS ?= 10000
TEMPORAL_VALIDITY_INDEX_BATCH_SIZE ?= 1000
TEMPORAL_VALIDITY_INDEX_PAYLOAD_RESIDENCY ?= lazy
GRAPH_INDEX_PERF_REPORT ?= target/graph-index-performance/report.json
GRAPH_INDEX_PERF_NODES ?= 100000
GRAPH_INDEX_PERF_SAMPLES ?= 31
GRAPH_INDEX_PERF_MAX_HOPS ?= 16
GRAPH_INDEX_PERF_VISIT_BUDGET ?= 4096
GRAPH_INDEX_PERF_MAX_P95_MS ?= 50
CONCURRENT_READ_BENCH_ROOT ?= target/concurrent-read-benchmark
CONCURRENT_READ_BENCH_REPORT ?= $(CONCURRENT_READ_BENCH_ROOT)/report.json
CONCURRENT_READ_BENCH_MARKDOWN ?= $(CONCURRENT_READ_BENCH_ROOT)/report.md
CONCURRENT_READ_BENCH_CELLS ?= 200
CONCURRENT_READ_BENCH_READER_THREADS ?= 1,2,4
CONCURRENT_READ_BENCH_READ_OPS_PER_THREAD ?= 10
CONCURRENT_READ_BENCH_WRITER_OPS ?= 5
CONCURRENT_READ_BENCH_MAX_P95_MS ?= 1000
INGESTION_THROUGHPUT_ROOT ?= target/ingestion-throughput
INGESTION_THROUGHPUT_REPORT ?= $(INGESTION_THROUGHPUT_ROOT)/report.json
INGESTION_THROUGHPUT_MARKDOWN ?= $(INGESTION_THROUGHPUT_ROOT)/report.md
INGESTION_THROUGHPUT_DOCS ?= 10000
INGESTION_THROUGHPUT_100K_ROOT ?= target/ingestion-throughput-100k
INGESTION_THROUGHPUT_100K_REPORT ?= $(INGESTION_THROUGHPUT_100K_ROOT)/report.json
INGESTION_THROUGHPUT_100K_MARKDOWN ?= $(INGESTION_THROUGHPUT_100K_ROOT)/report.md
INGESTION_THROUGHPUT_100K_DOCS ?= 100000
INGESTION_THROUGHPUT_BATCH_SIZE ?= 1000
INGESTION_THROUGHPUT_EMBEDDING_BATCH_SIZE ?= 128
INGESTION_THROUGHPUT_PAYLOAD_BYTES ?= 256
INGESTION_THROUGHPUT_MIN_DOCS_PER_SEC ?= 1
...
MEMORY_PROFILE_DIRECT_CHECKPOINT ?=
MEMORY_PROFILE_PAYLOAD_BYTES ?= 0
MEMORY_PROFILE_REOPEN_ONLY ?=
MEMORY_PROFILE_READ_SAMPLES ?= 0
...
SCALE_BENCH_10M_LAZY_REPORT ?= $(SCALE_BENCH_ROOT)/10m-lazy/report.json
SCALE_BENCH_10M_SAMPLES ?= 5
SCALE_BENCH_10M_BATCH_SIZE ?= 500000
SCALE_BENCH_10M_PAYLOAD_BYTES ?= 128
SCALE_BENCH_CI_10K_REPORT ?= $(SCALE_BENCH_ROOT)/ci-10k/report.json
SCALE_BENCH_CI_100K_REPORT ?= $(SCALE_BENCH_ROOT)/ci-100k/report.json
SCALE_BENCH_CI_SAMPLES ?= 5
SCALE_BENCH_CI_BATCH_SIZE ?= 50000
SCALE_BENCH_CI_PAYLOAD_BYTES ?= 128
...
CONTINUOUS_BENCHMARK_MIN_REGRESSION_DELTA_MS ?= 25
HOSTED_PERFORMANCE_HISTORY_ROOT ?= fixtures/performance/hosted-history
HOSTED_BENCHMARK_MIN_REGRESSION_DELTA_MS ?= $(CONTINUOUS_BENCHMARK_MIN_REGRESSION_DELTA_MS)
...
TIERED_STORAGE_V2_REPORT ?= target/tiered-storage-v2/report.json
