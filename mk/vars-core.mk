...
TRANSPARENCY_WITNESS_QUORUM_REPORT ?= target/transparency-witness-quorum/report.json
TRANSPARENCY_INCLUSION_REPORT ?= target/transparency-inclusion/report.json
TRANSPARENCY_CONSISTENCY_REPORT ?= target/transparency-consistency/report.json
TRANSPARENCY_AVAILABILITY_REPORT ?= target/transparency-availability/report.json
TRANSPARENCY_GOSSIP_REPORT ?= target/transparency-gossip/report.json
TRANSPARENCY_SLO_REPORT ?= target/transparency-slo/report.json
...
RANKING_FROZEN_WEIGHTS_FIXTURE ?= crates/cortex-engine/fixtures/ranking_frozen_weights_v1.json
RANKING_FROZEN_WEIGHTS_REPORT ?= target/ranking/frozen-weights-report.json
RANKING_WEIGHTS_DRIFT_REPORT ?= target/ranking/weights-drift-report.json
RANKING_WEIGHTS_DRIFT_GENERATED_ARTIFACT ?= target/ranking/generated-frozen-weights.json
RANKING_WEIGHTS_DRIFT_CALIBRATION_REPORT ?= target/ranking/weights-drift-calibration-report.json
RANKING_WEIGHTS_DRIFT_MODULE_REPORT ?= target/ranking/weights-drift-module-report.json
RANKING_LEARNED_LIFT_REPORT ?= target/ranking/learned-lift/report.json
RANKING_EXPLAIN_FAITHFULNESS_REPORT ?= target/ranking/explain-faithfulness/report.json
WEIGHTS_VERSION_BINDING_REPORT ?= target/determinism-hash/binding-report.json
PACK_DETERMINISM_HASH_REPORT ?= target/pack-determinism/report.json
SEMANTIC_MEMORY_COMPRESSION_REPORT ?= target/semantic-memory-compression/report.json
MULTI_AGENT_CONSISTENCY_REPORT ?= target/multi-agent-consistency/report.json
MULTI_AGENT_CLUSTER_CONSISTENCY_REPORT ?= target/multi-agent-cluster-consistency/report.json
HTTP_RAFT_ROUTING_ACCOUNTABILITY_REPORT ?= target/http-raft-routing-accountability/report.json
RAFT_INGRESS_PRODUCTION_GUARD_REPORT ?= target/raft-ingress-production-guard/report.json
RAFT_INGRESS_FORWARDING_REPORT ?= target/raft-ingress-forwarding/report.json
RAFT_INGRESS_LEADER_HINT_REPORT ?= target/raft-ingress-leader-hint/report.json
RAFT_INGRESS_AUTO_DISCOVERY_REPORT ?= target/raft-ingress-auto-discovery/report.json
RAFT_INGRESS_HEALTH_ROUTING_REPORT ?= target/raft-ingress-health-routing/report.json
RAFT_INGRESS_LIFECYCLE_MONITOR_REPORT ?= target/raft-ingress-lifecycle-monitor/report.json
RAFT_INGRESS_LOAD_POLICY_REPORT ?= target/raft-ingress-load-policy/report.json
RAFT_INGRESS_ADAPTIVE_SCHEDULING_REPORT ?= target/raft-ingress-adaptive-scheduling/report.json
RAFT_INGRESS_LOAD_METRICS_REPORT ?= target/raft-ingress-load-metrics/report.json
FORMAL_INVARIANTS_REPORT ?= target/formal-invariants/report.json
CANONICAL_SERIALIZATION_REPORT ?= target/canonical-serialization/report.json
...
COSINE_METRIC_CORRECTNESS_REPORT ?= target/cosine-metric-correctness/report.json
...
PACK_COMPLETENESS_SIGNAL_REPORT ?= target/pack-completeness/report.json
...
COMPLIANCE_CERTIFICATION_EXPECTED_FRAMEWORK ?=
COMPLIANCE_BOUNDARY_ARGS = $(if $(strip $(COMPLIANCE_CERTIFICATION_EVIDENCE)),--certification-evidence "$(COMPLIANCE_CERTIFICATION_EVIDENCE)",) $(if $(strip $(COMPLIANCE_CERTIFICATION_EXPECTED_FRAMEWORK)),--expected-framework "$(COMPLIANCE_CERTIFICATION_EXPECTED_FRAMEWORK)",)
COMPLIANCE_BOUNDARY_PRODUCTION_ORIGIN_ARGS = $(RECEIPT_PRODUCTION_ORIGIN_ARGS)
...
RECEIPT_KMS_HSM_CUSTODY_ARGS = $(if $(strip $(RECEIPT_KMS_HSM_CUSTODY_EVIDENCE)),--custody-evidence "$(RECEIPT_KMS_HSM_CUSTODY_EVIDENCE)",) $(if $(strip $(RECEIPT_KMS_HSM_EXPECTED_KEY_ID)),--expected-key-id "$(RECEIPT_KMS_HSM_EXPECTED_KEY_ID)",) $(if $(strip $(RECEIPT_KMS_HSM_EXPECTED_PUBLIC_KEY_HEX)),--expected-public-key-hex "$(RECEIPT_KMS_HSM_EXPECTED_PUBLIC_KEY_HEX)",) $(if $(strip $(RECEIPT_KMS_HSM_EXPECTED_SIGNER_REF)),--expected-signer-ref "$(RECEIPT_KMS_HSM_EXPECTED_SIGNER_REF)",)
...
RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_REF ?=
RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_PUBLIC_KEY_REF ?=
RECEIPT_KMS_HSM_CUSTODY_PRODUCTION_ORIGIN_ARGS = $(RECEIPT_PRODUCTION_ORIGIN_ARGS)
...
RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_REF ?=
RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_PUBLIC_KEY_REF ?=
RECEIPT_PRODUCTION_ORIGIN_ARGS = $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_TRUST_ANCHOR_EVIDENCE)),--trust-anchor-evidence "$(RECEIPT_PRODUCTION_ORIGIN_TRUST_ANCHOR_EVIDENCE)",) $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_KEY_ID)),--expected-key-attestor-key-id "$(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_KEY_ID)",) $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_PUBLIC_KEY_HEX)),--expected-key-attestor-public-key-hex "$(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_PUBLIC_KEY_HEX)",) $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_REF)),--expected-key-attestor-ref "$(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_REF)",) $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_PUBLIC_KEY_REF)),--expected-key-attestor-public-key-ref "$(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_KEY_ATTESTOR_PUBLIC_KEY_REF)",) $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_KEY_ID)),--expected-trust-anchor-publisher-key-id "$(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_KEY_ID)",) $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_PUBLIC_KEY_HEX)),--expected-trust-anchor-publisher-public-key-hex "$(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_PUBLIC_KEY_HEX)",) $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_REF)),--expected-trust-anchor-publisher-ref "$(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_REF)",) $(if $(strip $(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_PUBLIC_KEY_REF)),--expected-trust-anchor-publisher-public-key-ref "$(RECEIPT_PRODUCTION_ORIGIN_EXPECTED_TRUST_ANCHOR_PUBLISHER_PUBLIC_KEY_REF)",)
RECEIPT_PRODUCTION_READINESS_REPORT ?= target/receipt-production-readiness/report.json
RECEIPT_PRODUCTION_READY_REPORT ?= target/receipt-production-readiness/strict-report.json
RECEIPT_PRODUCTION_EVIDENCE_PREFLIGHT_REPORT ?= target/receipt-production-evidence/preflight.json
RECEIPT_PRODUCTION_EVIDENCE_HANDOFF_REPORT ?= target/receipt-production-evidence/handoff.json
RECEIPT_PRODUCTION_EVIDENCE_HANDOFF_CONSISTENCY_REPORT ?= target/receipt-production-evidence/handoff-consistency.json
PRODUCTION_EVIDENCE_ORIGIN_REPORT ?= target/production-evidence-origin/report.json
...
CONSENSUS_FAILOVER_BINDER_REPORT ?= $(CONSENSUS_GATE_ROOT)/failover-binder.json
...
CONSENSUS_RELEASE_LANE_RUNS ?= 3
CONSENSUS_RELEASE_LANE_ROOT ?= $(CONSENSUS_GATE_ROOT)/release-lane
CONSENSUS_RELEASE_LANE_REPORT ?= $(CONSENSUS_RELEASE_LANE_ROOT)/report.json
