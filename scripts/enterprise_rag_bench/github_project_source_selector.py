#!/usr/bin/env python3
"""GitHub project-chain source selector for EnterpriseRAG-Bench."""
...
from github_project_modes import MODE_CONFIG
...
def selector_mode(question: dict[str, Any]) -> str | None:
    if question.get("question_type") != "project_related":
        return None
    if "github" not in {str(item) for item in question.get("source_types", [])}:
        return None
...
class SourceIndex:
    def __init__(self, *, uuid_index: dict[str, str], sources_dir: Path) -> None:
        ...
        self.github_paths = [
            rel_path for rel_path in sorted(uuid_index.values()) if rel_path.startswith("github/")
        ]

    def candidate_docs(self, mode: str) -> list[tuple[str, str, str]]:
        markers = MODE_CONFIG[mode]["markers"]
        docs: list[tuple[str, str, str]] = []
        for rel_path in self.github_paths:
...
def score_doc(mode: str, question_text: str, doc: tuple[str, str, str]) -> int:
    ...
    score = score_terms(text, terms) + 10 * score_terms(rel_path, terms)
    return score + 160 * sum(1 for marker in config["markers"] if marker in rel_path)
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for qid, base_row in sorted(baseline.items()):
        ...
        if mode:
            ...
            output["route"] = {"enabled": True, "mode": mode, "policy": args.policy_name, "source": "github_project_source_selector"}
        else:
            output["document_ids"] = baseline_ids
            output["route"] = {"enabled": False, "policy": args.policy_name, "source": "github_project_source_selector"}
    ...
    report = {
        ...
        "schema_version": "cortexdb.enterprise_rag_bench.github_project_source_selector.v1",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--policy-name", default="github_project_source_selector_v1")
