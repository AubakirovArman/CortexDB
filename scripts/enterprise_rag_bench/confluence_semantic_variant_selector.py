#!/usr/bin/env python3
"""Confluence semantic variant selector for EnterpriseRAG-Bench.

Targets semantic Confluence questions where the baseline selects nearby pages
but misses the exact policy/runbook variant. It uses question text and local
Confluence source fields only: no LLM/API calls and no gold-aware doc IDs.
"""
...
from confluence_semantic_variant_modes import MODE_CONFIG
...
class SourceIndex:
    def __init__(self, *, uuid_index: dict[str, str], sources_dir: Path) -> None:
        ...
        self.confluence_paths = [
            rel_path for rel_path in sorted(uuid_index.values()) if rel_path.startswith("confluence/")
        ]
        self._confluence_docs: list[tuple[str, str, dict[str, Any], str]] | None = None

    def confluence_docs(self) -> list[tuple[str, str, dict[str, Any], str]]:
        if self._confluence_docs is not None:
            return self._confluence_docs
...
def mode_quality_gate(mode: str, text: str, rel_path: str) -> bool:
    lower = text.lower()
    path = rel_path.lower()
    if mode == "restoration_sprint_sequence":
        return "ten-step restoration sprint" in lower and has_any(lower, ("60–90", "60-90"))
    if mode == "status_wire_cadence":
        return "status-wire" in lower and "tribe liaison" in lower and "containment" in lower
    if mode == "employee_lifecycle_turnaround":
        return "employee lifecycle" in lower and "accounts" in lower and has_any(lower, ("emea/apac", "apac"))
    if mode == "operational_policy_gallery":
        return "operational flows" in lower and "policy gallery" in lower and "templates" in lower
    if mode == "offer_midpoint_approval":
        return "offer evaluation" in lower and "band_midpoint" in lower and "hrbp" in lower
    if mode == "ops_orchestra_routine_approvals":
        return "ops orchestra" in lower and "target sla for routine approvals" in lower
...
def top_confluence_docs(index: SourceIndex, mode: str, question_text: str) -> list[str]:
    ...
    for doc in index.confluence_docs():
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for qid, base_row in sorted(baseline.items()):
        ...
        if mode:
            ...
            output["route"] = {"enabled": True, "mode": mode, "policy": args.policy_name, "source": "confluence_semantic_variant_selector"}
        else:
            output["document_ids"] = baseline_ids
            output["route"] = {"enabled": False, "policy": args.policy_name, "source": "confluence_semantic_variant_selector"}
    ...
    report = {
        ...
        "schema_version": "cortexdb.enterprise_rag_bench.confluence_semantic_variant_selector.v1",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--policy-name", default="confluence_semantic_variant_selector_v1")
