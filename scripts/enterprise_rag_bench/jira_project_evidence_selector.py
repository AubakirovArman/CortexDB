#!/usr/bin/env python3
"""Jira project evidence selector for EnterpriseRAG-Bench.

Targets project-related questions where a customer-support Jira ticket is part
of the evidence chain but is lost by final top10 composition. It uses question
text and local Jira source fields only: no LLM/API calls and no gold-aware doc
IDs.
"""
...
from jira_project_evidence_modes import MODE_CONFIG
...
def jira_text(rel_path: str, payload: dict[str, Any]) -> str:
    values = [
        ...
        payload.get("linked_issues"),
        payload.get("related_github_prs"),
        payload.get("related_confluence_pages"),
...
def mode_quality_gate(mode: str, text: str, rel_path: str) -> bool:
    lower = text.lower()
    path = rel_path.lower()
    if mode == "invoice_retry_double_count":
        return (
            "double-charged" in lower
            and has_any(lower, ("retry/fallback", "retry fallback", "streaming retry"))
            and "credit" in lower
            and "billing" in lower
        )
    if mode == "burst_noisy_neighbor_latency":
        return (
            "noisy-neighbor" in lower
            and "non-burst" in lower
            and "burst window" in lower
            and has_any(lower, ("slo", "circuit"))
        )
    if mode == "demo_dashboard_empty_metrics":
        return (
            "demo tenant" in lower
            and "console dashboards" in lower
            and "empty graphs" in lower
            and "synthetic traffic" in lower
        )
...
def select_docs(mode: str, question_text: str, baseline_ids: list[str], index: SourceIndex, limit: int) -> list[str]:
    promoted = top_jira_docs(index, mode, question_text)
    customer_jira = [
        doc_id
        for doc_id in baseline_ids
        if index.uuid_index.get(doc_id, "").startswith("jira/customer-support/")
    ]
    internal_jira = [
        doc_id
        for doc_id in baseline_ids
        if index.uuid_index.get(doc_id, "").startswith("jira/internal-support/")
    ]
    other_docs = [
        doc_id
        for doc_id in baseline_ids
        if not index.uuid_index.get(doc_id, "").startswith("jira/")
    ]
    if mode == "demo_dashboard_empty_metrics":
        return unique(promoted + customer_jira + internal_jira + other_docs)[:limit]
    return unique(promoted + customer_jira + other_docs + internal_jira)[:limit]
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for qid, base_row in sorted(baseline.items()):
        ...
        if mode:
            ...
            output["route"] = {"enabled": True, "mode": mode, "policy": args.policy_name, "source": "jira_project_evidence_selector"}
        else:
            output["document_ids"] = baseline_ids
            output["route"] = {"enabled": False, "policy": args.policy_name, "source": "jira_project_evidence_selector"}
    ...
    report = {
        ...
        "schema_version": "cortexdb.enterprise_rag_bench.jira_project_evidence_selector.v1",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--policy-name", default="jira_project_evidence_selector_v1")
