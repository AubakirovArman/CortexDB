...
pub struct AppState {
    ...
    request_id_client_provided: Arc<AtomicU64>,
    request_id_generated: Arc<AtomicU64>,
        ...
        Ok(body_str) => {
            ...
            if method == "GET" && path == "/v1/metrics" {
                if query.contains("format=prometheus") {
                    ...
                    let request_id_client_provided =
...
cortexdb_compaction_input_bytes_total {}\n\
                ...
                if let Ok(mut metrics) = serde_json::from_str::<MetricsResponse>(&body_str) {
                    ...
                    metrics.request_id_client_provided =
                        state.request_id_client_provided.load(Ordering::Relaxed);
                    metrics.request_id_generated =
                        state.request_id_generated.load(Ordering::Relaxed);
...
enum RequestIdSource {
    Client,
    Generated,
}

fn request_id_from_headers(headers: &axum::http::HeaderMap) -> RequestIdInfo {
