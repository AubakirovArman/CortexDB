...
use cortex_engine::{evaluate_ann_drift_baseline, AnnDriftBaseline};

const DEFAULT_BASELINE_PATH: &str = "crates/cortex-engine/fixtures/ann_drift_baseline_v1.json";
const DEFAULT_OUTPUT_PATH: &str = "target/ann/ann_drift_report.json";
...
fn run() -> Result<(), String> {
    ...
    let baseline: AnnDriftBaseline = serde_json::from_slice(&bytes)
        .map_err(|error| format!("failed to parse {}: {error}", args.baseline_path.display()))?;
    let report = evaluate_ann_drift_baseline(&baseline)
        .map_err(|error| format!("failed to evaluate ANN drift baseline: {error}"))?;
    ...
    fn parse(args: impl IntoIterator<Item = String>) -> Result<Self, String> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--help" | "-h" => {
                    return Err(format!(
                        "usage: ann_drift_check [--baseline PATH] [--output PATH]\n\
