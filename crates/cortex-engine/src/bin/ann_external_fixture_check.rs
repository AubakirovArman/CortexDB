...
use cortex_engine::{evaluate_ann_external_fixture, AnnExternalFixtureBaseline};

const DEFAULT_BASELINE_PATH: &str = "crates/cortex-engine/fixtures/ann_external_baseline_v1.json";
const DEFAULT_OUTPUT_PATH: &str = "target/ann/ann_external_fixture_report.json";
...
fn run() -> Result<(), String> {
    ...
    let baseline: AnnExternalFixtureBaseline = serde_json::from_slice(&baseline_bytes)
    ...
    let report = evaluate_ann_external_fixture(&baseline, &fixture_text)
        .map_err(|error| format!("failed to evaluate ANN external fixture: {error}"))?;
    ...
    if report.passed {
        ...
        Err(format!(
            "ANN external fixture failed for {}: {}",
            report.baseline_id,
    ...
    fn parse(args: impl IntoIterator<Item = String>) -> Result<Self, String> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--help" | "-h" => {
                    return Err(format!(
                        "usage: ann_external_fixture_check [--baseline PATH] [--output PATH]\n\
