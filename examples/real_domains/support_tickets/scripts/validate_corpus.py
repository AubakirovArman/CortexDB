#!/usr/bin/env python3
"""Validate the support_tickets retrieval corpus."""
...
def main() -> int:
    ...
    for row in docs:
        ...
        for field in ["doc_id", "source", "tenant", "title", "product", "priority", "url", "text"]:
    ...
    for row in chunks:
        ...
        for field in ["chunk_id", "doc_id", "source", "tenant", "product", "priority", "title", "text"]:
            require_text(row, field, str(label))
    ...
    print(f"support_tickets corpus valid: documents={len(docs)} chunks={len(chunks)}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"support_tickets corpus validation failed: {error}", file=sys.stderr)
        raise SystemExit(1)
