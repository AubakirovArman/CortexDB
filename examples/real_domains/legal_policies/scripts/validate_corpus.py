#!/usr/bin/env python3
"""Validate the legal_policies retrieval corpus."""
...
def main() -> int:
    ...
    for row in docs:
        ...
        for field in ["doc_id", "source", "jurisdiction", "title", "policy_area", "url", "text"]:
    ...
    for row in chunks:
        ...
        for field in ["chunk_id", "doc_id", "source", "jurisdiction", "policy_area", "title", "text"]:
            require_text(row, field, str(label))
    ...
    print(f"legal_policies corpus valid: documents={len(docs)} chunks={len(chunks)}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"legal_policies corpus validation failed: {error}", file=sys.stderr)
        raise SystemExit(1)
