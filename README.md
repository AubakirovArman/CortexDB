...
- `crates/cortex-engine`: single-node database loop, incremental checkpoint, compaction, AQL-backed retrieve, AQL `REMEMBER`, memory TTL/decay, durable feedback, source trust, `VERIFY FACT` reports, ContextPack v0, scoped search, search helpers with hybrid fusion, ACLOG-backed replication log entries, and consensus model primitives.
