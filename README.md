...
- `crates/cortex-engine`: single-node database loop, incremental checkpoint, compaction, AQL-backed retrieve, AQL `REMEMBER`, memory TTL/decay, durable feedback, source trust, `VERIFY FACT` reports with citation/numeric guards, structured and conservative natural-language contradiction hints, ContextPack v0, scoped search, analyzer packs, persisted HNSW-backed vector search with delete/rebuild maintenance, ingestion adapters with simple native PDF/Flate extraction, ACLOG-backed replication log entries, token-authenticated TCP/in-memory replication transports, snapshot chunks, and consensus model primitives.
