...
| A3.1 Graph-descent HNSW insert | Deferred (reverted) | Prototyped a full standard-HNSW insert (deterministic entry point → greedy upper-layer descent → `ef_construction` beam per layer → diversity neighbor-selection heuristic + pruning), replacing the O(N²) full-scan build. Verified byte-identical double build, bounded degree, integrity after build+remove, and **recall@10 = 0.95 vs exact on realistic high-dim clustered data**. **Not shipped**: pure diversity is too sparse on the dim-8 synthetic production gate (recall 0.57 vs the exact builder's 0.75), and every densification attempt (hnswlib `keepPrunedConnections`, M0=2·M, larger ef) collapsed navigability. Reverted to keep the safety gate intact rather than weaken it. | A correct `keepPrunedConnections` that preserves bridge edges in low dimensions (or a higher-dim representative fixture) is needed before this meets the plan's "within 1 point" bar. Blocks A3.2/A3.3. | `hnsw-build-scaling-check` (planned) |
