...
| A6.4 Honest ERB run (exit-proof) | Slice (measured, not passing) | Honest EnterpriseRAG-Bench retrieval measurement **through the `cortex_engine::Database` product path** (runner `cortex-engine-keyword-source-aware-retrieval`), full **511,958-doc** corpus, balanced-50: **recall@10 = 56.35%** (47 answerable), recall@20 = 59.54%. Strong on factual pools (basic 76.5%, intra-doc reasoning 75%, conflicting-info 100%), weak on `semantic` (23.1%) and `project_related` (24.7%) — consistent with the A2.0 caveat. Pipeline re-verified end-to-end on 2026-07-02 (smoke, exit 0). Recorded honestly in [`ERB_ENGINE_PATH_RETRIEVAL.md`](ERB_ENGINE_PATH_RETRIEVAL.md): the retrieval exit bar (≥75% recall@10) is **not yet met**, and answer correctness/completeness is `0.0` because this is a retrieval-only run (no LLM answer generated). | Reaching ≥75% recall needs A1.4/A4.2/A7.2 (the still-open plan deps) and wiring the default-off A3.1/A5/A4.1 slices into the bench path; the answer+judge combined-score half of the exit-proof is the outstanding (expensive) external Gemma run. | `enterprise-rag-bench-cortexdb-retrieval-50` |
