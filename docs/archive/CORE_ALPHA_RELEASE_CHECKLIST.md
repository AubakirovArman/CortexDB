...
| Production evidence sweep | `make production-evidence-sweep` writes `target/production-evidence/report.json` and per-step logs for OpenAPI contract, backup drill, single-node performance, tenant recovery, ANN release evidence, ANN 10-run production SLO history, real-embedding readiness, and replication partition checks. |
...
| Migration compatibility fixture | `make migration-compatibility-check` validates `fixtures/migration/compatibility_matrix_v1.json`, restores historical backup fixtures, and writes `target/migration-upgrade-matrix-v2/report.json` proving previous-release direct database open/write/backup/restore with the current binary. |
