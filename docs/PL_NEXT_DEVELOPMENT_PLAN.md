...
- Added historical backup fixture generated by the `v0.1.0-core-alpha.5`
  tagged CLI under `fixtures/migration/historical/v0.1.0-core-alpha.5/`.
- `make migration-compatibility-check` now restores historical backup fixtures
  with the current `cortexdb` binary and writes
  `target/migration-historical-restore/report.json`.
- `fixtures/migration/compatibility_matrix_v1.json` records the historical
  restore fixture alongside storage/API/SDK compatibility boundaries.
- Migration docs now require historical restore fixture coverage before
  compatibility claims are accepted.
- `make migration-policy-check`, `make migration-compatibility-check`, and
  `make storage-compat-check` passed locally.
