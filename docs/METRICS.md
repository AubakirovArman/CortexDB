...
| `cortexdb_request_timeout_total` | Prometheus-only counter for requests that hit a configured body-read timeout or a read/admin actor timeout. | Any increase means request admission or a cancellable read/admin route exceeded its configured budget. Admitted mutations wait for a definitive outcome and are not detached behind a timeout response. |
| `request_duration_ms_total` | Sum of request durations in ms. | Use with request count for rough mean latency. |
| `request_id_client_provided` | Safe `x-request-id` headers accepted from clients. | Shows external log-correlation adoption. |
| `request_id_generated` | Request IDs generated by CortexDB for missing or unsafe client IDs. | High values mean clients are not providing correlation IDs. |
...
rejection, route timeout, ANN fallback, ANN latency buckets, and
...
Request-id source is also emitted as the low-cardinality Prometheus series
`cortexdb_request_id_source_total{source="client"}` and
`cortexdb_request_id_source_total{source="generated"}`. The raw request ID is
not used as a metric label.
...
- `cortexdb_request_timeout_total > 0`: inspect slow clients, request body
  upload behavior, and route timeout budgets before raising concurrency.
