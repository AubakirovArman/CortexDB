...
- **Repeatable ANN gate:** `make ann-fixture-check` runs the deterministic synthetic corpus in release mode and compares recall, graph shape, upper-layer shape, latency ceilings, and `production_safe` against `crates/cortex-engine/fixtures/ann_fixture_baseline_v1.json`. `make ann-fixture-report` writes the same JSON to `target/ann/ann_fixture_report.json`, and CI uploads it as an artifact for commit-to-commit drift checks.
