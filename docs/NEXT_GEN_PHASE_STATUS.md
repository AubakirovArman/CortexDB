...
> - **Official metered-run data** (3): A6.3, F2.3, F3.3. (**F3.1 now landed** — its fast lane is a self-tested per-type regression gate over a committed mini-log, the same offline pattern as F2.2/F5.2; `longmemeval-per-type-regression-check`.)
