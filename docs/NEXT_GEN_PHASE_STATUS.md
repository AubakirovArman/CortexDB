...
| Phase | Title | State | Golden | Next action / blocker |
| --- | --- | --- | --- | --- |
| A6.1 | Judge the existing b8 artifact (81.07% recall) end-to-end | interim-done (3-judge) | none | **Three-judge interim cross-check landed** (2026-07-04): the 500 committed `erb-submission/answers.jsonl` re-judged with the unchanged official-clean 2-axis rubric by three independent judges — gemini-3.5-flash **47.74** (recorded), Gemma-31B **46.71**, gpt-oss-120B **41.82**. Deterministic doc-recall identical across all three (55.71%). The LLM-judged combined is **judge-dependent within a ~42–48 band**, and the *stronger* model (gpt-oss-120B) is the *stricter* judge (~5–6pt lower) — so the honest signal is "combined ~42–48, stronger judge trends lower," **not** "judge-agnostic ~47." Implication: the official gpt-5.4/GPT-4o (stronger still) may land at/below the band, not above. `leaderboard_comparable=false`; see docs/archive/GEMMA_BOUNDED_INTERIM_RUNS.md §A6.1 + `gemma_interim/erb_a61_{gemma,gptoss120b}_judge_500.json`. **DoD remainder:** the leaderboard-official evaluator (no budget). |
