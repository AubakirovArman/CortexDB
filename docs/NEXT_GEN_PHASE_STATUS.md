...
> - **A3.3-impl** (1): the one substantial remaining *engineering* task (not a metered-infra blocker — I over-stated that; the 50k benchmark is local compute). It is genuinely L-effort and frozen-format-touching: gate the per-query exact recall-recompute (`ann/search.rs:229`) behind `should_sample_recall` (default-off, so goldens are untouched), **persist a per-collection `RecallWindow` in the manifest** (a storage-format change requiring the storage-format-freeze protocol), wire the exact-serving fallback on `is_degraded`, add the receipt `serving_epoch` (the C3-5 additive-minor bump, per the ADR), and prove exact-scans ≤15% / p50 ≥3× / corrupted-graph degradation / double-run determinism on a generated 50k-vector fixture. The primitives (`guarded_recall.rs`) + design ADR + primitive gate (`ann-guarded-sampling-check`) are done; the wiring + persistence + benchmark are a careful multi-step effort, not a rushed end-of-turn push (it touches the ANN core + two frozen formats + many existing gates).
