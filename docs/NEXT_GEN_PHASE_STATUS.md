...
> - **A3.3-impl** (1): the one substantial remaining *engineering* task — genuinely buildable here (I over-stated both "metered-infra" and "signed-golden-regen-blocked"; neither holds). **Two sub-blockers dissolved this session:** the design gate (C3-5 ADR) and now the **receipt-visibility golden-safety** — `serving_epoch` is additive in the determinism input (`None` on the default path → the committed accountability golden verifies byte-identically; `Some` adds `ann_serving_epoch` + changes the hash), proven by `determinism_hash::tests::serving_epoch_is_additive_and_golden_safe` with the accountability goldens still green. **Remaining (the L-effort bulk):** thread a per-collection `RecallWindow` + sampling counters through the (currently stateless) ANN search path to gate the per-query exact recall-recompute at `ann/search.rs:220` behind `should_sample_recall`, wire the exact-serving fallback on `is_degraded`, persist the window additively in the manifest (storage-format care like the A2.1 EMBD section), thread `serving_epoch` from that path into the receipt, and prove exact-scans ≤15% / p50 ≥3× / corrupted-graph degradation / double-run determinism on a generated 50k-vector fixture. This is a stateful architectural change across the ANN core + manifest + a slow benchmark — a genuine multi-increment effort, now unblocked with the foundation laid.
