...
> - **Official metered-run data** (2): A6.3, F2.3. (**F3.1 and F3.3 now landed** — both are self-tested regression gates with a fast offline lane over committed mini-logs, the same pattern as F2.2/F5.2; `longmemeval-per-type-regression-check`, `multihop-retrieval-regression-check`. Their real-run `--log` paths plug in the official data when a metered run lands.)
