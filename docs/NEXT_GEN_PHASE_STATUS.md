...
> - **F4.3 (pgvector+OPA snapshot matrix) — reclassified as buildable here, NOT external-operator-blocked.** This environment has a working docker (28.5.1) with registry pull access, so the "needs an operator machine with docker" precondition is satisfiable in-session. F4.3 is genuinely unbuilt (no `scripts/aab/adapters/pgvector_opa/`, no `capture_snapshot`/`score_snapshot`, no snapshot fixtures) — it is a substantial from-scratch epic: design the empirical AAB-mini test set (corpus + queries + gold for axes 1–4, since F4.2 hand-assigned the `thin-pgvector-opa` scores), build the pgvector+OPA docker adapter + OPA `.rego` policies, `capture_snapshot` the outputs once, and a fully-offline `aab-snapshot-matrix-check` gate that re-scores the committed snapshot (axes 1–4 from captured outputs; axes 5–6 UNRANKED with reason codes). Correctness-sensitive (OPA authz + empirical scoring) — warrants a focused build, but is the clearest remaining phase closeable with in-session resources (deterministic metric, no LLM judge).
