...
> - **A3.3-impl** (1): the one substantial remaining *engineering* task — genuinely buildable here (I over-stated both "metered-infra" and "signed-golden-regen-blocked"; neither holds). **Engineering complete this session (~12 gated increments); 3 of 4 quantitative DoD criteria proven + receipt-visibility + persistence.** Beyond the functional core below, also landed: **`serving_epoch`→receipt threading** (receipt-visibility DoD; accountability goldens byte-identical) and **additive manifest persistence** of the window (`ManifestGuardedRecallState` "AGRS" section, golden-safe append-when-`Some`; storage-format-freeze green; window survives restart via `from_parts`). The one remaining DoD criterion, **p50 ANN ≥3×**, is now **measured and root-caused** (`ann-guarded-sampling-latency-bench`, `CORTEXDB_BENCH_N` configurable): **1.1× at both n=5000 and n=50000** (p50 exact 154ms vs ann-only 135ms at 50k), with the skip-path confirmed serving via `HnswGraph` (not fallback). The ann-only cost grew ~8× from 5k→50k — **roughly linear in n** — so the engine's persisted ANN search path is **O(n)-dominated, not O(log n)**; the exact scan A3.3 skips (~19ms) is a small fraction of the ANN path's own O(n) cost. So **p50≥3× is not reachable at the current engine ANN-search performance — a pre-existing `search_persisted_ann` traversal/setup characteristic, not a gap in A3.3's feature** (flagged as a separate investigation, task_8552e290). The **deterministic half** — exact-scans ≤15% — is proven. Net: A3.3's guarded-sampling feature is fully implemented (wired, default-off byte-identical, durable, receipt-visible), 3/4 quantitative DoD criteria proven; the 4th is blocked by an out-of-scope O(n) ANN-search perf issue.

**Functional-core detail (superseded by the line above):** Built + tested: golden-safe receipt `serving_epoch` (goldens byte-identical) · `GuardedRecallState` machine · the ANN-only skip path (`search_persisted_ann_sampled`/`RecallMode`, Exact byte-identical) · `AnnGuardedSamplingOptions` knob + `Mutex<Option<GuardedRecallState>>` on `Database` (default-off) · the live read-path wiring (`Database::guarded_ann_search`: per-query sample decision, rebuild detection via manifest generation, degraded→exact serving; **150 search + 37 query_search tests + `ann-metric-matrix-check` confirm default-off is byte-identical**) · exact-scan-rate telemetry. **DoD criteria proven** (`ann-guarded-sampling-check`): degradation latches ≤8 sampled queries; double-run determinism; **exact-scans ≤15%** (1306 bps over 5000 queries vs the pre-A3.3 100%). **Remaining (3 pieces):** the **p50 ANN ≥3× timing benchmark** (a local corpus benchmark — timing, so nightly not a deterministic gate), threading `serving_epoch` into the receipt during retrieval (the golden-safe hook exists), and additive manifest persistence of the window (`from_parts` round-trip). Superseding the earlier note: the deterministic core is done, wired, and default-off safe.
