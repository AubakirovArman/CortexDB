...
- tasks:
  ...
  - [ ] 4) partial: AQL/get/search/session/temporal/tool/graph paths materialize payload through `Database::payload_for_version`; corpus synonym and search context expansion stores still need lazy-aware hydration or descriptor/index-backed design
...
- next exit step: add lazy-aware secondary store hydration or make those stores descriptor/index-backed, then run the lazy crash matrix and memory/latency evidence before marking A08 done.
