...
- next exit step: publish 1M-cell fresh-process RSS evidence before marking A08 done; then extend the broader crash/corruption/restart matrix to lazy-mode parity or split the remaining parity work into a dedicated follow-up if 1M evidence is accepted first.
