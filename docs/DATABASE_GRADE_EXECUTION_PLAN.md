...
- acceptance:
  - [ ] 1) `grep snapshot_versions` по запросным путям = 0 — `query.rs` is covered by `indexed-retrieve-gate-check`; broader search/context query-adjacent paths are now inventoried by `query-scan-inventory-check` as 1 query-adjacent call and 8 maintenance/backfill calls.
