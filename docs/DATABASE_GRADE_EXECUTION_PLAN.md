...
- tasks:
  ...
  - [ ] 4) partial: AQL/get/search/session/temporal/tool/graph/search-context/corpus-synonym paths materialize payload through `Database::payload_for_version`; remaining verification evidence/execution/graph/conflict helpers still need lazy-aware hydration or descriptor/index-backed design
  - [ ] 5) partial: core restart matrix covers lazy checkpoint/compact/WAL-tail paths; full fault-injection matrix still pending.
- acceptance:
...
- next exit step: add lazy-aware hydration for remaining verification evidence/execution/graph/conflict helpers, then run the lazy crash matrix and memory/latency evidence before marking A08 done.
