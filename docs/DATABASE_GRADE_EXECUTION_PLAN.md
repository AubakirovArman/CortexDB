...
- latest evidence: Added `scripts/scale_benchmark_inventory.py` to inventory existing A19 reports without running a new heavy benchmark. The current inventory report `target/scale-bench/inventory.json` is `partial` with 14 reports found. It proves core lifecycle coverage for 100K and 1M and ContextPack p50/p95 coverage for 100K and 1M. It explicitly identifies remaining A19 gaps: 100K/1M keyword_search p50/p95, 100K/1M verify_fact p50/p95, 10M post-lazy RSS/latency, and multi-point before/after optimization trend curves.
