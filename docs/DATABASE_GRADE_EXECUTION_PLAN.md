...
- tasks:
  ...
  - [ ] 2) матрица: open time, RSS, put/get/search/context/verify p50/p95, checkpoint time — на 100K/1M (10M — после A08) — 100K/1M core lifecycle matrix is reproducible; 100K search/verify p50/p95 are captured; 1M search/verify p50/p95 remain open.
...
- latest evidence: Added `scripts/scale_benchmark_inventory.py` to inventory existing A19 reports without running a new heavy benchmark. The current inventory report `target/scale-bench/inventory.json` is `partial` with 16 reports found. It proves core lifecycle coverage for 100K and 1M, ContextPack p50/p95 coverage for 100K and 1M, and keyword/VerifyFact p50/p95 coverage for 100K. The controlled 100K direct-checkpoint run at `target/scale-bench/a19-search-verify-100k/report.json` passed with fixed 128-byte payloads, `keyword_search.p50_ms=1307.151`, `keyword_search.p95_ms=1307.151`, `verify_fact.p50_ms=4077.174`, and `verify_fact.p95_ms=4077.174`. Remaining A19 gaps: 1M keyword_search p50/p95, 1M verify_fact p50/p95, 10M post-lazy RSS/latency, and multi-point before/after optimization trend curves.
