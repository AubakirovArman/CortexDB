...
| --- | --- | --- | --- | --- |
| Dynamic RBAC | Defer full dynamic RBAC to beta. | Static `admin`/`data` route roles, file-backed token rotation, optional token-to-AgentView binding. | Durable policy store, policy mutation API, disabled principal lifecycle, role hierarchy. | Block any release or docs that claim enterprise RBAC, dynamic user management, or external identity integration. |
