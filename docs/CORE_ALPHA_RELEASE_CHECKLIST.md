...
| ANN release package | `make ann-publish-baseline`, `make ann-package-baseline`, and `make ann-validate-baseline-package` produce and verify a `.tar.gz` with `package_manifest.json`, SHA-256 file checksums, `history.json`, generated ground truth, and `production_safe=true`. |
