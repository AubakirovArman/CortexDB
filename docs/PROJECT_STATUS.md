...
- **Core Alpha HTTP Safety Controls:** Optional Bearer auth, optional single-token `AgentView` binding for scope-aware data routes, bounded actor queues with explicit `database_busy` backpressure, optional fixed-window rate limiting, exact-origin CORS allowlisting, and opt-in route-level audit logging with an optional synced JSONL file sink.
...
- **Security Model:** Core Alpha has local safety controls, but not full multi-user RBAC, per-user quotas, tamper-evident audit trails, at-rest encryption, or distributed security hardening.
