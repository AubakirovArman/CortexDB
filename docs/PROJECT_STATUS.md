...
- **Asynchronous Network Surface:** Async HTTP server built on Axum and Tokio. Blocking DB transactions run through `tokio::task::spawn_blocking`; the database core itself remains local and blocking.
...
- **Ecosystem:** Python and TypeScript SDK client foundations, local dataset fixture loaders, and complete automated verification gates (`make alpha-check`).
