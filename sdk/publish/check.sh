...
cargo test -p cortex-api-types -p cortexdb-sdk --all-features --manifest-path "$REPO_ROOT/Cargo.toml"
cargo package -p cortex-api-types --manifest-path "$REPO_ROOT/Cargo.toml" --allow-dirty >/dev/null
# cortexdb-sdk depends on cortex-api-types. Cargo resolves versioned dependencies
...
if [ "${CORTEX_API_TYPES_PUBLISHED:-0}" = "1" ]; then
  cargo package -p cortexdb-sdk --manifest-path "$REPO_ROOT/Cargo.toml" --allow-dirty >/dev/null
else
  printf 'skipping cortexdb-sdk package verification until cortex-api-types is published\n'
fi
